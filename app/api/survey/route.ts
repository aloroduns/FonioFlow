...
export async function GET(request:Request){const measure=optionalText(request,"measure");const envelope=await resolveDataset({cacheKey:"survey",source:"Fonio Market Research Survey (150 submitted records)",staticData:h6.survey,staticTimestamp:"2026-09-11",schema});const filtered=filterEnvelope(envelope,row=>!measure||row.Measure.toLowerCase().includes(measure.toLowerCase()));return jsonResponse({...filtered,meta:{...filtered.meta,warnings:[...filtered.meta.warnings,h6.surveyAudit.warning]}})}
