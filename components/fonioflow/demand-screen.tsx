...
export function DemandScreen(){
  ...
  return <div className="page">
    <ScreenTitle code="H6" title="Demand & Domestic Availability" text="Combine production, reported trade, population and 150 submitted survey records with an explicit duplication warning."/>
    ...
    <div className="metric-grid four"><Metric label="Submitted survey records" value={`${submitted}`}/><Metric label="Aware of fonio" value={share(aware)}/><Metric label="Likely purchase in 3 months" value={share(purchaseIntent)}/><Metric label="Farmer-support influence" value={share(farmerInfluence)}/></div>
    <div className="availability"><div className="chart-card"><h3>Latest availability per person</h3><ResponsiveContainer width="100%" height={330}><BarChart data={latest} layout="vertical"><CartesianGrid strokeDasharray="3 3"/><XAxis type="number"/><YAxis type="category" dataKey="country" width={90}/><Tooltip/><Bar dataKey="availability_kg_per_person" fill="#A45147" radius={[0,8,8,0]}/></BarChart></ResponsiveContainer></div><div className="survey-card"><h3>Survey findings</h3>{keyFindings.map(item=><div key={item.label}><span>{item.label}</span><strong>{share(item.value)}</strong></div>)}<p>{fonioData.surveyAudit.exact_duplicate_rows} exact duplicate rows and only {fonioData.surveyAudit.unique_timestamps} unique timestamps were detected. Results describe the submitted file and cannot support population-level inference.</p></div></div>
  </div>;
}
